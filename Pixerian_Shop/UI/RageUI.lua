...
AddEventHandler('RageUI.LoadMenus', function()
    RageUI.Loaded = true
end)
...
RageUI.Settings = {
    ...
    Items = {
        Title = {
            ...
            Text = { X = RageConfig.Title.x or 215, Y = RageConfig.Title.y or 20, Scale = 1.15 },
        },
        Subtitle = {
            ...
            Background = { Width = 0, Height = 20 },
            Text = { X = 15, Y = -35, Scale = 0.35 },
            PreText = { X = 425, Y = 3, Scale = 0.0 },
        },
        ...
        if CurrentMenu.Sprite ~= nil then
            if CurrentMenu.Sprite.Dictionary ~= nil then
                if CurrentMenu.Sprite.Dictionary == "commonmenu" then
                    Graphics.Sprite(CurrentMenu.Sprite.Dictionary, CurrentMenu.Sprite.Texture, CurrentMenu.X, CurrentMenu.Y, RageUI.Settings.Items.Title.Background.Width + CurrentMenu.WidthOffset, RageUI.Settings.Items.Title.Background.Height, 0,CurrentMenu.Sprite.Color.R, CurrentMenu.Sprite.Color.G, CurrentMenu.Sprite.Color.B, CurrentMenu.Sprite.Color.A)
                    RageUI.Glare()
                else
                    Graphics.Sprite(CurrentMenu.Sprite.Dictionary, CurrentMenu.Sprite.Texture, CurrentMenu.X, CurrentMenu.Y, RageUI.Settings.Items.Title.Background.Width + CurrentMenu.WidthOffset, RageUI.Settings.Items.Title.Background.Height, nil, nil, nil, nil, 240)
                    RageUI.Glare()
                end
            else
...
function RageUI.Glare()
    --GLARE
    if RageConfig.Header.GLARE then
        local CurrentMenu = RageUI.CurrentMenu
        ...
        ---@type number
        local GlareX = CurrentMenu.X / 1860 + (RageUI.GetSafeZoneBounds().X / (64.399 - (CurrentMenu.WidthOffset * 0.065731)))
        ---@type number
        local GlareY = CurrentMenu.Y / 1080 + RageUI.GetSafeZoneBounds().Y / 33.195020746888
        RageUI.SetScaleformParams(ScaleformMovie, {
            { name = "SET_DATA_SLOT", param = { GetGameplayCamRelativeHeading() } }
        })

        DrawScaleformMovie(ScaleformMovie, GlareX, GlareY, Glarewidth / 430, Glareheight / 100, 255, 255, 255, 255, 255)
    end
    --END GLARE
end
...
function RageUI.Subtitle()
    ...
    if (CurrentMenu.Display.Subtitle) then
        RageUI.ItemsSafeZone(CurrentMenu)
        --if CurrentMenu.Subtitle == nil then CurrentMenu.Subtitle = "ㅤ" end
...
function RageUI.Render(onCloseMenu)
    local CurrentMenu = RageUI.CurrentMenu;
...
function RageUI.ItemsDescription(Description)
    -- todo fix height of description when linecount > 4
    local CurrentMenu = RageUI.CurrentMenu;
...
function RageUI.getListNumber(min, length, speed)
    local data = {}
    for i = min, length, speed or 1 do
        table.insert(data, i)
    end
    return data
end
